...
### Simulation コンペのフェーズ指針（エージェント提出型）

Simulation コンペ（Lux AI / ConnectX / Halite など）には固定 train/test も CV も無い。「検証」は環境で相手と対戦することそのもの。フェーズも「CV を伸ばす」ではなく「**相手プールへの勝率と提出レートを伸ばす**」で読み替える。

- **序盤 (~30%)**:
  - **do**: 動く提出パイプライン（1エピソード完走するエージェント）/ ヒューリスティック baseline / 環境とI/Fの把握 / ローカル評価ハーネス + 相手プール初版
  - **don't**: 序盤から RL を大回しする / 探索の重い実装に深入りする / 相手1種だけに過学習する
- **中盤 (30-70%)**:
  - **do**: 探索（minimax/MCTS）や RL（self-play/PPO）の強化 / 相手プールの多様化 / リプレイでの負け分析（どの局面で負けるか帰納的に分類）
  - **don't**: 相手1種だけで評価を回す / 評価ハーネス無しの体感チューニング
- **終盤 (70%-)**:
  - **do**: 方策アンサンブル（相手別カウンター・状況別の使い分け）/ メタ変化への追従 / LB shake 評価 / 最終エージェント選定
  - **don't**: 新規アルゴリズム着手 / 大きな観測・行動設計の変更

進捗判定は CV の代わりに「**固定相手プールに対する勝率の推移 + 提出レートの推移**」で行う。詳細は CLAUDE.md の「フェーズ別ガード」simulation 版を参照。
...
- simulation コンペでは「アンサンブル」は予測の平均ではなく、**方策の混合・相手別の使い分け**を指す（局面や対戦相手に応じて複数の方策・探索深さを切り替える）。ここでも単体方策の勝率を測ってから組み合わせるのは同じ
