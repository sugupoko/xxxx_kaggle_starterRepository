...
# kaggle CLI が無ければ案内して終了（無いまま進むと意味不明なエラーになる）
if ! command -v kaggle >/dev/null 2>&1; then
  echo "Error: kaggle CLI が見つかりません。pip install kaggle で導入してください。" >&2
  exit 1
fi

if [ ! -d "$1" ]; then
  echo "Error: フォルダが見つかりません: $1" >&2
  exit 1
fi
...
# （set -e により kaggle コマンドが失敗したら成功メッセージを出さずに即終了する）
